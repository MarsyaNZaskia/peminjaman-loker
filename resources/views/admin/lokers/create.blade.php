...
<div class="max-w-2xl mx-auto px-4">
    <h1 class="text-3xl font-bold mb-6">Tambah Loker Baru</h1>

    <div class="bg-white p-6 rounded-lg shadow">
        <form method="POST" action="{{ route('admin.lokers.store') }}">
            @csrf

            <div class="mb-4">
                <label class="block text-gray-700 mb-2">Nomor Loker</label>
                <input type="text" name="nomor_loker" value="{{ old('nomor_loker') }}" 
                       placeholder="Contoh: L1-001" 
            ...
            <div class="mb-6">
                ...
                <textarea name="keterangan" rows="3" 
                          ...
                          placeholder="Catatan tambahan tentang loker...">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
