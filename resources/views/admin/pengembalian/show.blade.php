...
<div class="max-w-5xl mx-auto px-4 py-8">
    ...
    {{-- Sama seperti view petugas, tinggal copy paste dari petugas/pengembalian/show.blade.php --}}
    {{-- Atau buat @include('petugas.pengembalian._detail', ['pengembalian' => $pengembalian]) --}}
    ...
    <div class="bg-slate-900/50 backdrop-blur-xl border border-white/10 p-6 rounded-2xl shadow-2xl mb-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="col-span-2 bg-white/5 p-4 rounded-xl border border-white/5">
                ...
                <p class="font-medium text-slate-300 leading-relaxed">{{ $pengembalian->peminjaman->keperluan }}</p>
            </div>
        </div>
    </div>
                ...
                <div class="bg-white/5 p-5 rounded-2xl border border-white/5">
                    ...
                    <p>
                        @if($pengembalian->kondisi_barang === 'baik')
                            <span class="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-lg shadow-emerald-500/10">Kondisi Baik</span>
                        @elseif($pengembalian->kondisi_barang === 'rusak')
                            <span class="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 shadow-lg shadow-amber-500/10">Buku Rusak</span>
                        @else
                            <span class="px-4 py-1.5 text-[10px] font-black uppercase tracking-widest rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-lg shadow-rose-500/10">Buku Hilang</span>
                        @endif
                    </p>
                </div>
                <div class="bg-white/5 p-5 rounded-2xl border border-white/5">
